<template>
  <div class="book-session">
    ...
    <ul class="mb-4">
      ...
      <li>
        Время:
        <b>{{ formatTime(detailSession?.startTime as string) }}</b>
      </li>
    </ul>
      ...
      >
        ...
        >
          <div class="pr-2 text-right text-sm">
            <span class="cinema-row">ряд {{ r }}</span>
          </div>
          <button
            v-for="s in cols"
            :key="`r${r}-s${s}`"
    ...
    <div v-if="selectedSeats.length > 0" class="mt-6 text-center">
      ...
      <button class="px-6 py-2 button text-white" @click="confirmBooking">
        Забронировать
      </button>
    </div>
  </div>
...
const cols = computed<number>(
  () => detailSession.value?.seats?.seatsPerRow ?? 0,
);
...
onBeforeMount(async () => {
  try {
    ...
    if (cinemaStore.cinemas.length === 0) {
