...
import { BaseHttpError, ValidationError, UnauthorizedError, ConflictError, ServerError, NetworkError } from '../../core/domain/shared/HttpErrors'
...
describe('HttpErrorHandler.handle', () => {
    ...
    it('должен бросать NetworkError для статуса 0 (сетевая ошибка)', () => {
        ...
        try {
            ...
            expect((error as NetworkError).message).toBe('Проверьте подключение к интернету')
